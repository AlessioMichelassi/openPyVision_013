...
from mainDir.inputs.testClass.uMat_vs_NumpyTest.baseUMatClass import UMAt_BaseClass


# Questa classe gestisce la cattura dello schermo
class Umat_ScreenCapture(UMAt_BaseClass):

    def __init__(self, synchObject, screen_index, resolution=QSize(1920, 1080), region=None, targetFps=60):
        super(Umat_ScreenCapture, self).__init__(synchObject, resolution)
        ...
        self._frame = cv2.UMat(np.zeros((resolution.height(), resolution.width(), 3), dtype=np.uint8))  # Frame iniziale vuoto
    ...
    def capture_frame(self):
        ...
        if self.camera:
            ...
            if frame is not None:
                ...
                self._frame = cv2.UMat(frame)  # Convert frame to UMat
        else:
            print("Camera not initialized")
    ...
    def frameProcessor(self, frame):
        # Sovrascrivi il metodo per assicurarti che le operazioni avvengano su UMat
        if frame is not None:
