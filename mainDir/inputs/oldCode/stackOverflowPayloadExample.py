...
class RandomNoiseImageGenerator(QObject):
    ...
    def generate_noise(self):
        ...
        return np.ascontiguousarray(np.random.randint(0, 256, (height, width, 3), dtype=np.uint16))
    ...
    def getFrame(self):
        ...
        if self._frame is None:
            return np.ones((self.resolution.height(), self.resolution.width(), 3), dtype=np.uint16) * 255
        return self._frame
