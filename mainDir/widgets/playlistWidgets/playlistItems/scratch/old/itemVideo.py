...
class ItemVideo(QWidget):
    ...
    def getVideoInfo(self, path):
        ...
        if audio_info.returncode == 0:
            ...
            channels = audio_info_dict.get('channels', '0')
            duration = float(audio_info_dict.get('duration', '0'))
            minutes, seconds = divmod(int(duration), 60)
            self.lblAudioCodec.setText(f"Audio: {sample_rate}Hz {channels}ch duration {minutes:02}:{seconds:02}")

            # Calculate average volume per channel
            avg_volume_per_channel = self.calculate_average_volume(path, channels)
            self.lblAudioCodec.setText(f"{self.lblAudioCodec.text()} - Avg Vol: {avg_volume_per_channel}")

        # Check for black frames at start and end
        start_black_frame = self.check_black_frame(path, start=True)
        end_black_frame = self.check_black_frame(path, start=False, fps=fps, duration_seconds=duration_seconds, duration_minutes=duration_minutes)
        if start_black_frame:
            self.lblVideoCodec.setText(f"{self.lblVideoCodec.text()} - black frame at start - first frame {start_black_frame}")
        if end_black_frame:
    ...
    def calculate_average_volume(self, path, channels):
        """
        Calculate the average volume for each audio channel
        :param path: Path to the video file
        :param channels: Number of audio channels
        :return: List of average volumes per channel
        """
        ...
        cmd = [
            ...
            f'volumedetect=print:file={tmpfile_path}', '-vn', '-sn', '-f', 'null', '-'
        ]
        ...
        with open(tmpfile_path, 'r') as f:
            for line in f:
                if 'mean_volume' in line:
                    volume = float(line.split(':')[-1].strip().replace(' dB', ''))
